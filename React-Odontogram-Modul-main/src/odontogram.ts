...
function buildSelect(selectEl: Any, options: Any, onChange: Any){
  selectEl.innerHTML = "";
  for(const opt of options){
    const labelText = opt.labelKey ? t(opt.labelKey) : opt.label;
    const o = el("option", { value: opt.value, text: labelText });
...
export async function exportImage(format: "png" | "jpg" = "png", download = true){
  ...
  try{
    ...
    if(format === "jpg"){
      ...
      if(download) downloadDataUrl(dataUrl, `odontogram-${stamp}.jpg`);
    }else{
      dataUrl = canvas.toDataURL("image/png");
      if(download) downloadDataUrl(dataUrl, `odontogram-${stamp}.png`);
    }
